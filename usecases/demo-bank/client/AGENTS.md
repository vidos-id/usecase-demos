...
- Do NOT edit `src/routeTree.gen.ts` (auto-generated). Regenerate routes via `bun run build:demo-bank-client` from repo root.
...
- Keep parsing strictly typed with shared Zod schemas from `demo-bank-shared/api/authorization-sse` and `demo-bank-shared/api/callback-sse`.
