...
- [ ] 1.1 Add shared debug SSE schemas in `shared/src/api/` as discriminated unions with typed envelope fields (`eventType`, `level`, `timestamp`, `message`, scoped identifiers, payload).
...
- [ ] 1.3 Add channel-separation contract definitions so debug payloads and business status payloads cannot be mixed.

## 2. Server Debug Stream and Emitters

- [ ] 2.1 Implement debug SSE route/channel using shared SSE transport primitives (serializer, keepalive, disconnect cleanup).
- [ ] 2.2 Add server-side subscription scoping that binds each debug stream to caller session and request correlation.
- [ ] 2.3 Add typed debug emitter helpers for key domains (auth lifecycle, callback resolution, Vidos calls, store state transitions).
- [ ] 2.4 Enforce server-side filtering so subscribers receive only events owned by their session/request scope.
...
- [ ] 5.4 Run `bun run check-types` and `bun run lint` and fix issues introduced by this change.
