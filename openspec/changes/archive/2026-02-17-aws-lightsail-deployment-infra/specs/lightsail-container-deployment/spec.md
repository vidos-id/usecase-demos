...
The Lightsail service SHALL be configured as a micro tier service with a single node, exposing port 53913 via a public HTTPS endpoint.

#### Scenario: Service endpoint enabled

- **WHEN** the Lightsail service is created
- **THEN** a public HTTPS endpoint is enabled for port 53913
