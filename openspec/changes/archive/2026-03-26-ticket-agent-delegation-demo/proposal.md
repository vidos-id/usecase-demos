...
- Add a server application (`ticket-agent/server/`): Bun + Hono HTTP API on a unique subdomain that handles user signup/signin, PID identity verification, delegation credential issuance via `@vidos-id/issuer`, event catalog, and booking creation with delegation credential verification via Vidos Authorizer.
...
- Add an agent skill file (`skill.md`): instructions for OpenClaw agents describing the HTTP API endpoints, `wallet-cli` usage for credential import and OpenID4VP presentation, and the end-to-end delegation flow.
...
- Add `@vidos-id/issuer` as a new dependency from GitHub Packages for server-side credential issuance.
...
- `ticket-agent-credential-issuance`: Server-side credential issuance service using `@vidos-id/issuer` to produce holder-bound `dc+sd-jwt` delegation credentials, binding to the agent's public key via direct `cnf` embedding without requiring a proof JWT.
- `ticket-agent-agent-skill`: OpenClaw skill file describing the HTTP API endpoints, `wallet-cli` integration for credential import and `openid4vp://` presentation, and the complete agent-side delegation and purchasing flow.
...
- New external dependency `@vidos-id/issuer` from GitHub Packages under the `@vidos-id` scope. Requires `bunfig.toml` or `.npmrc` registry configuration for the scope in the monorepo.
...
- Agent tooling depends on `wallet-cli` being available as an OpenClaw tool. The agent uses `wallet-cli init` (to generate its holder key), `wallet-cli import` (to store the delegation credential), and `wallet-cli present` (to create and submit a VP in response to an `openid4vp://` authorization URL).
